...
const SchemaContact= require('../Model/contact')
...
ContactRoute.get('/getcontact',async (req,res)=> {
    try{
        const contact=  await SchemaContact.find()
...
ContactRoute.post('/addcontact',async (req,res)=> {
    try{
        const newContact= new SchemaContact(req.body)
...
ContactRoute.put('/updatecontact/:id',async (req,res)=> {
    try{
        const {id}= req.params
        const updateduser=   await SchemaContact.findByIdAndUpdate(id,{$set:{...req.body}})
...
ContactRoute.delete('/deletecontact/:id',async (req,res)=> {
    try{
        const {id}= req.params
        const deleteduser=   await SchemaContact.findByIdAndDelete(id)
...
ContactRoute.get('/getUnqcontact/:id',async (req,res)=> {
    try{
        const {id}= req.params
        const getUnquser=   await SchemaContact.findById(id)
